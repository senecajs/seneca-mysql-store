...
const intern = {
  ...
  async insertent (args, ctx) {
    ...
    const inserted = await intern.insertrow({
      into: ent_table,
    ...
    return intern.loadent({
      ent,
      where: { id: ent.id || inserted.insertId }
    }, ctx)
  },
  ...
  make_insertable(msg) {
    const { ent, q } = msg
    const { auto_increment$: auto_increment = false } = q


    // WARNING: It looks like Entity#clone$ does not clone private fields
    // of the parent entity, which may lead to some confusion.
    //
    const out = ent.clone$()

    out.id = out.id || ent.id$ || intern.generateid().id
  ...
  async do_create(msg, ctx) {
    ...
    const new_ent = intern.make_insertable(msg)
