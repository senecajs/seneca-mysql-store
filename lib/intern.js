...
const intern = {
  ...
  async do_create(msg, ctx) {
    ...
    if (null != upsert_fields) {
      return intern.upsertent(upsert_fields, { ent: new_ent }, ctx)
    }
    */

    return intern.insertent({ ent: new_ent }, ctx)
  },
  ...
  async do_update(msg, ctx) {
    ...
    if (!updated_anything) {
      return intern.insertent({ ent }, ctx)
    }


    return intern.loadent({ ent, where: { id: ent.id } }, ctx)
  },
  ...
  async upsertent(upsert_fields, args, ctx) {
    ...
    return intern.transaction(async (trx) => {
      ...
      // TODO: TODO:
      //
      const ins_sel_query = 'select 1' //QueryBuilder.insertwherenotexistsstm(ent, update_q)
      //
      await intern.execquery(ins_sel_query, trx_ctx)
