...
module.exports.makeent = function (ent, row) {
  //console.log('inside makeent')
  //console.dir(row, { depth: 32 }) // dbg
  ...
  */
  ...
  if (!_.isUndefined(ent) && !_.isUndefined(row)) {
    fields.forEach(function (field) {
      ...
      // TODO: Pass the schema, and cast if a column type is JSON
      /*
  ...
  const result = ent.make$(entp)

  //console.log('makeent result:') // dbg
  //console.dir(result, { depth: 32 }) // dbg
...
module.exports.escapeColumn = function (col) {
  return '`' + module.exports.escapeStr(col) + '`'
}
