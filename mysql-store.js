...
module.exports = function(options) {
  ...
  internals.connectionPool = {
    query: function(query, inputs, cb) {
      ...
      if (inputs === undefined) {
        return internals.connectionPool.query(
          internals.connectionPool,
          query,
          report
        )
    ...
    escape: function() {
      return internals.connectionPool.escape.apply(
        internals.connectionPool,
        arguments
      )
    },
  ...
  var reconnect = function() {
    configure(internals.spec, function(err) {
      if (err) {
        seneca.log(
          'db reconnect (wait ' + internals.opts.minwait + 'ms) failed: ',
          err
        )
        internals.waitmillis = Math.min(
          2 * internals.waitmillis,
          internals.opts.maxwait
        )
        setTimeout(function() {
          reconnect()
  ...
  var store = {
    ...
    load: function(args, done) {
      ...
      seneca.act(
        { role: actionRole, hook: 'load', target: store.name },
        args,
        function(err, queryObj) {
          var query = queryObj.query
    ...
    list: function(args, done) {
      ...
      seneca.act(
        { role: actionRole, hook: 'list', target: store.name },
        args,
        function(err, queryObj) {
          var query = queryObj.query
    ...
    remove: function(args, cb) {
      ...
      function executeRemove(args, row) {
        seneca.act(
          { role: actionRole, hook: 'remove', target: store.name },
          args,
          function(err, queryObj) {
            var query = queryObj.query
            if (err) {
  ...
  seneca.add({ role: actionRole, hook: 'save' }, function(args, done) {
    ...
    seneca.act(
      { role: actionRole, hook: 'generate_id', target: args.target },
      function(err, result) {
        if (err) {
  ...
  seneca.add(
    { role: actionRole, hook: 'generate_id', target: store.name },
    function(args, done) {
